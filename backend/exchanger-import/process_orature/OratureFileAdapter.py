...
class OratureFileAdapter:
    ...
    def __init__(self, directory):
        ...
        self.file_name_regex = r"^(\w){2,}_(\w){2,}_(\w){3}_c(\d){1,3}_v(\d){1,3}_t(\d){1,3}.wav$"
        self.SELECTED_TAKES_MAPPER = "selected.txt"

    def _add_zero_padding(self, file_name):
        if re.match(self.file_name_regex, file_name):
    ...
    def _build_media_manifest(self, selected_takes=[]):
        ...
        for base_dir, dirs, files in os.walk(self._root):
            for f in files:
                ...
                if not re.match(self.file_name_regex, f):
                    continue
...
# OratureFileAdapter(r"E:\Shared\vi-ulb-sng-test").process()
